...
| :--- | :------- | :----------- |
| `quality-checks.sh` | `Stop` | When `.ts/.tsx` files are dirty: `lint:fix` → `lint` → `typecheck` → tests for affected packages. Blocks (exit 2) on any failure. No-ops when no TS changed. |
| `convention-spot-check.sh` | `Stop` | Advisory scan of changed TS: `export default`, inline types in service/route files, missing JSDoc (`packages/` only), non-`Readonly` React props, `EventEmitter` in the session engine, direct `new XService()` in the gateway, verbose comments. Always advisory (exit 0). |
