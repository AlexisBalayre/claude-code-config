...
- `docs/conventions/*`: the primary, authoritative rules. The universal `core.md` applies to every source file, `testing.md` to tests, and each other area doc to the paths its `.claude/rules/*-conventions.md` loader names; each auto-loads when you read a file in that area. Weight these first. A section still marked `TODO(adapt)` states no rule.
